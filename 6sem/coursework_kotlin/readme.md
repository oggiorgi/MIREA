...
Сервер раздаёт статические файлы из директорий:

| Путь | Директория | Содержимое |
|------|------------|------------|
| `/uploads/audio/*` | `uploads/audio/` | MP3-файлы треков |
| `/uploads/covers/*` | `uploads/covers/` | JPG-изображения обложек |
...
|-------------|----------|
| **JWT-аутентификация** | Токен генерируется при входе, содержит userId и login. Срок действия — 7 дней |
| **BCrypt хеширование** | Пароли не хранятся в открытом виде, используется соль для защиты от радужных таблиц |
| **CORS** | Настроен для доступа с любых хостов, разрешены методы GET, POST, PUT, DELETE, OPTIONS |
| **Пул соединений HikariCP** | Оптимизирует работу с PostgreSQL, поддерживает до 10 одновременных подключений |
| **Статические файлы** | Аудио и обложки раздаются через встроенный статический сервер Ktor |
| **Обработка конфликтов** | При создании плейлиста с уже существующим именем возвращается HTTP 409 |
